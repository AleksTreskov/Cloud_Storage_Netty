...
public class ServerHandler extends ChannelInboundHandlerAdapter {
    ...
    public ServerHandler() {
        this.logger = LoggerFactory.getLogger(ServerHandler.class);
    ...
    private void processMsg(AbstractMsg msg, ChannelHandlerContext ctx) {
        ...
        if (isLogged) {
            if (msg instanceof FileSendingMsg) {
                saveFileToStorage((FileSendingMsg) msg);
            ...
            if (msg instanceof AuthorizationMsg) {
                System.out.println("Nickname in CloudServerHandler" +
                        ((AuthorizationMsg) msg).getNickname());
                checkAuth((AuthorizationMsg) msg, ctx);
            }
        }
    }
    ...
    private void sendFile(CommandMsg msg, ChannelHandlerContext ctx) {
        try {
            ...
            sendData(
                    new FileSendingMsg(filePath), ctx);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private void saveFileToStorage(FileSendingMsg msg) {
    ...
    private void checkAuth(AuthorizationMsg incomingMsg, ChannelHandlerContext ctx) {
        if (incomingMsg != null) {
